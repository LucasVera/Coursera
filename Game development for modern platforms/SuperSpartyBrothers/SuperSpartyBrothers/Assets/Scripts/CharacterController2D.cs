...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
		// first, check if player is hiding and arrow key was released.
		// If this is the case, allow movement and continue as normal.
		if (isHiding && (
		...
		// if player presses 'down' button, then start hiding animation. also, block movement
		if (isGrounded == true && (
		...
		// start hiding animation and ignore collisions between enemies and player!
		_animator.SetBool ("Hiding", isHiding);
		Physics2D.IgnoreLayerCollision (_playerLayer, _enemyLayer, isHiding);

		// determine horizontal velocity change based on the horizontal input
		//_vx = Input.GetAxisRaw ("Horizontal");
		_vx = CrossPlatformInputManager.GetAxisRaw ("Horizontal");
		// Determine if running based on the horizontal movement
		if (_vx != 0 && playerCanMove) 
		{
			isRunning = true;
		} else {
			isRunning = false;
		...
		if(isGrounded && CrossPlatformInputManager.GetButtonDown("Jump") && playerCanMove) // If grounded AND jump button pressed, then allow the player to jump
		{
			doJump ();
		//}else if (_canDoubleJump && Input.GetButtonDown ("Jump")) 
		}else if (_canDoubleJump && CrossPlatformInputManager.GetButtonDown ("Jump") && playerCanMove) 
		{
			doJump ();
			_canDoubleJump = false;
		}
